...
Furthermore, several variables were plotted against classe, and some were evidently showing difference between different classes. As an example, the median of the roll_forearm variable is clearly higher for classe C.

```{r, EDAplot, fig.align = "center", out.width = "50%", fig.cap = "**Fig.1**: *Box and whisker showing the spread of the roll_forearm data across the 5 classes.*"}
...
```{r, Confusion_rpart, echo = FALSE}
library(kableExtra)
CM_count_rpart <- confusionMatrix(reduced_train$classe, predict(cv_rpart_Fit, newdata = reduced_train))
knitr::kable(CM_count_rpart$table, align = 'l', caption = 
        "**Table 1**: Confusion table for regression with Recursive Partitioning.") %>% 
        kable_styling(full_width = F) %>% add_header_above(c("Observed" = ncol(CM_count_rpart$table)+1)) %>%
        group_rows("Predicted",1,5) %>% column_spec(1, bold = TRUE, width = "7em")
```

```{r, Confusion_LDA, echo = FALSE}
library(kableExtra)
CM_count_LDA <- confusionMatrix(reduced_train$classe, predict(fit_LDA, newdata = reduced_train))
knitr::kable(CM_count_LDA$table, align = 'r', caption = 
        "**Table 2**: Confusion table for regression with Linear Discriminant Analysis.") %>% 
        kable_styling(full_width = F) %>% add_header_above(c("Observed" = ncol(CM_count_LDA$table)+1)) %>%
        group_rows("Predicted",1,5) %>% column_spec(1, bold = TRUE, width = "7em")
```
...
```{r, Confusion_gbm}
#knitr::kable(CM_B, align = 'c', caption = "**Table 3**: Confusion table for regression with gradient Boosting Machine.") %>% kable_styling(full_width = F)
```

Employing a Gradient Boosting Machine (*gbm*) gives an accuracy of `r Acc_gbm`%, which is quite an improvement.
...
```{r, Confusion_rf}
#knitr::kable(CM_rf, align = 'c', caption = "**Table 4**: Confusion table for regression with Random Forest.") %>% kable_styling(full_width = F)
```

Random Forest provides average accuracy from cross validation of `r Acc_rf`%. Instead of the Out Of Sample error, the Out Of Bag error is estimated, yielding a value of `r OOB_rf`%. These metrics show that model obtained with random forest method is satisfactory.
